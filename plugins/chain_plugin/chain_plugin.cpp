...
abi_def get_abi( const controller& db, const name& account ) {
   ...
   EOS_ASSERT(code_accnt_metadata != nullptr, chain::account_query_exception, "Fail to retrieve metadata account for ${account}", ("account", account) );
   abi_def abi;
...
read_only::get_code_results read_only::get_code( const get_code_params& params, const fc::time_point& )const {
   ...
   if( accnt_metadata.code_hash != digest_type() ) {
      const auto& code_obj = d.get<code_object, by_code_hash>(accnt_metadata.code_hash);
...
read_only::get_raw_code_and_abi_results read_only::get_raw_code_and_abi( const get_raw_code_and_abi_params& params, const fc::time_point& )const {
   ...
   result.abi = blob{{accnt_metadata.abi.begin(), accnt_metadata.abi.end()}};
...
read_only::get_raw_abi_results read_only::get_raw_abi( const get_raw_abi_params& params, const fc::time_point& )const {
   ...
   const auto& d = db.db();
   const auto& accnt_metadata     = d.get<account_metadata_object,by_name>(params.account_name);
   result.abi_hash = fc::sha256::hash( accnt_metadata.abi.data(), accnt_metadata.abi.size() );
   if( accnt_metadata.code_hash != digest_type() )
      result.code_hash = accnt_metadata.code_hash;
